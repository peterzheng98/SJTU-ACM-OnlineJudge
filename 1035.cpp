...
public:
	...
	{
		Node *t = rank(r,root);
		if(t == NULL) fprintf(stderr,"Input error! %d\n",r);
		rank(r,root,true);
		return t->data;
	}

	Node* rank(int r, Node *n, bool debug=false)
	{
		if(n == NULL) return NULL;
		int s = size(n->right_child);
		if(debug) printf("%d",r);
		if(s >= r)
			return rank(r,n->right_child,debug);
		if(s + 1 == r)
			return n;
		return rank(r-s-1,n->left_child,debug);
	}
